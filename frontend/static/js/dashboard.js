...
function updateChart(data) {
    // Guard: chart must be initialized and have pages data
    if (!perfChart || !data || !data.pages) return;

    const labels = data.pages.map(p => 'Page ' + p.page);
    const changes = data.pages.map(p => ((1 - (p.image_similarity || 0)) * 100).toFixed(1));
    const confidence = data.pages.map(p => p.confidence ? (p.confidence * 100).toFixed(0) : 90);

    perfChart.data.labels = labels;
    perfChart.data.datasets[0].data = changes;
    perfChart.data.datasets[0].label = 'Change %';
    perfChart.data.datasets[1].data = confidence;
...
function addActivity(initials, bg, name, desc) {
    ...
    if (!feed) return; // Guard: activity feed may not exist on all views
...
// Sync user info if already available
